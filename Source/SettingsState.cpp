...
, mHardButton()
{
	...
	mDifficultyLabel = std::make_shared<GUI::Label>("Difficulty", *context.fonts);
	mDifficultyLabel->setPosition(80.f, 60.f);
	mGUIContainer.pack(mDifficultyLabel);

	mSimpleButton = std::make_shared<GUI::Button>(context);
	mSimpleButton->setPosition(80.f, 100.f);
	mSimpleButton->setText("Simple");
	mSimpleButton->setCallback([this, context]()
	{
		*context.difficulty = State::Simple;
		updateDifficultyLabel();
	});

	mHardButton = std::make_shared<GUI::Button>(context);
	mHardButton->setPosition(290.f, 100.f);
	mHardButton->setText("Hard");
	mHardButton->setCallback([this, context]()
	{
		*context.difficulty = State::Hard;
		updateDifficultyLabel();
	});

	mGUIContainer.pack(mSimpleButton);
	mGUIContainer.pack(mHardButton);
	...
	for (std::size_t x = 0; x < 2; ++x)
	{
		...
		addButtonLabel(PlayerAction::LaunchBomb,	x, 6, "Bomb", context);
	}
...
void SettingsState::updateDifficultyLabel()
{
	if (*getContext().difficulty == State::Simple)
		mDifficultyLabel->setText("Difficulty: Simple");
	else
		mDifficultyLabel->setText("Difficulty: Hard");
}
